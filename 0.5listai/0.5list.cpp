...
#include "struktura.h"
void pildymas(list<studentas>& a) //pildo studentų sąrašą
{
    studentas b;
    int dydis = 0;
    string s, arrandom;
...
void rusiavimas(list<int>& lst, list<int>::iterator low, list<int>::iterator high) {
    if (std::distance(low, high) >= 1) {
        list<int>::iterator pivot = low;
        list<int>::iterator i = std::next(low);
        list<int>::iterator j = high;
        ...
        rusiavimas(lst, low, std::prev(j));
        rusiavimas(lst, std::next(j), high);
    }
}
...
void merge(std::list<studentas>& A, int low, int mid, int high) {
    int n1 = mid - low + 1;
    int n2 = high - mid;

    std::list<studentas> left;
    std::list<studentas> right;
    ...
    auto listIt = A.begin();
    std::advance(listIt, low);

    while (i < n1 && j < n2) {
        if (!palyginimas(*leftIt, *rightIt)) {
            *listIt = *leftIt;
            ++leftIt;
            ++i;
        }
...
void rusiavimasStudentu(std::list<studentas>& A, int low, int high) 
{
    if (low < high) {
        int mid = low + std::distance(A.begin(), A.end()) / 2;
        ...
        std::copy(A.begin(), it, std::back_inserter(left));
        std::copy(it, A.end(), std::back_inserter(right));

        // Recursively sort the two halves
        rusiavimasStudentu(left, low, mid);
        rusiavimasStudentu(right, mid + 1, high);
...
void galutinisVid(studentas& A)
{
    ...
    for (it = A.paz.begin(); it != A.paz.end(); ++it) {
        sum += *it;
    }
...
void galutinisMed(studentas& A) {
    ...
    std::list<int>::iterator it = A.paz.begin();
    std::advance(it, vidurinis);

    if (A.paz.size() % 2 == 0) {
        med = (*std::prev(it) + *it) / 2.0;
    }
    else {
        med = *it;
    }
...
void Spausdinimas(const std::list<studentas>& A) {
    cout << setw(10) << "Vardas" << setw(15) << "Pavarde" << setw(15) << "Vidurkis" << setw(15) << "Mediana" << endl;
    cout << "---------------------------------------------------------------\n";

    for (const auto& student : A) {
        std::cout << std::setw(10) << student.vardas << setw(15) << student.pavarde << setw(15) << fixed << setprecision(2) << student.vidurkis << setw(15) << fixed << setprecision(2) << student.mediana << endl;
    }
}
