...
void rusiavimas(vector<int>& arr, int low, int high) {
    if (low < high) {
        int pivot = low;
        int i = low + 1;
        int j = high;
...
void merge(vector<studentas>& A, int low, int mid, int high) {
    int n1 = mid - low + 1;
    int n2 = high - mid;

    std::vector<studentas> left(n1);
    std::vector<studentas> right(n2);
...
void rusiavimasStudentu(vector<studentas>& A, int low, int high) {
    if (low < high) {
